...
**Why:** Pre-launch hardening removed a shipped admin backdoor (a "Method D" magic-phone + hardcoded password/verify-code, plus a `+200000000001` bypass test phone). The bundle had real secrets a decompiler could read. Editing client code alone does NOT close the hole: a matching Firebase **test phone** is a server-side backdoor, and `firestore.rules` are inert until `firebase deploy --only firestore:rules`.
